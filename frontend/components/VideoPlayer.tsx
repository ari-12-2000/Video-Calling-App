...
export default function VideoPlayer({
  ...
  if (!stream || videoOff) {
    return (
      <div className={`h-full aspect-9/16 grid items-center bg-black rounded-lg ${remoteOrientation === "landscape"? 'max-sm:h-[50vh] -translate-[50%]':''}`}>
        <FallbackAvatar />
      </div>
    );
  }
  ...
  return (
    <video
      ...
      playsInline
      muted={muted}
      className={`bg-black rounded-lg object-cover h-full ${orientationClass} ${remoteOrientation === "landscape"? 'max-sm:h-[50vh] -translate-[50%]':''}` }
    />
  );
}
